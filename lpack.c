...
int tolua_unpack(lua_State *L) 		/** unpack(s,f,[init]) */
{
 size_t len;
...
int tolua_pack(lua_State *L) 		/** pack(f,...) */
{
 int i=2;
...
void tolua_openpack(lua_State *L)
{
  lua_register(L,"bpack",tolua_pack);
  lua_register(L,"bunpack",tolua_unpack);
}
